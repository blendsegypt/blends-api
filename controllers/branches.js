...
router.post("/", async (req, res) => {
  try {
    ...
    // Add inventory records for all retail products
    const retailProducts = await DB.Product.findAll({
      where: {
        retail: true,
      },
    });
    ...
    const newInventory = [];
    retailProducts.forEach(async (product) => {
      newInventory.push({
    ...
    await DB.Inventory.bulkCreate(newInventory);
...
router.put("/:id", async (req, res) => {
  try {
    ...
    // update branch
    const [numberOfAffectedRows] = await DB.Branch.update(branchUpdates, {
      where: {
        id: req.params.id,
      },
      returning: true,
